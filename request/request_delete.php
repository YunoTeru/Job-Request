<?php
    ...
    $req_id = $_GET['id'];
    $request = $requestdao->retrieveSingleRequest($req_id);
    if(isset($_POST['submit'])){
        $confirm  = $_POST['confirm'];
        if($confirm == 'delete'){
            deleteRequest($cust_id);
...
<body>
    <div class="conteainer">
        <div class="card border-danger m-3 p-3">
            <h4 class="card-title">Are you sure you`re going to delete this request?</h4>
            <p class="card-text">Please type 'delete' if you are sure. If not, type 'cancel'</p>
            <p class="card-text">
                Details <br>
                Request Name: <?php echo $request['req_name']; ?><br>
                Request Date Added: <?php echo $request['req_date']; ?><br>
            </p>
            <form method="post">
